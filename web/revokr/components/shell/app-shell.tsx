...
export function AppShell({ children, ...sidebarProps }: AppShellProps) {
  ...
  return (
    <>
      ...
      {/* A floating glass sidebar, inset from the window edges like macOS. */}
      <aside className="glass-rim fixed inset-y-3 left-3 z-30 hidden w-60 overflow-hidden rounded-[26px] bg-white/[0.035] shadow-[inset_0_1px_0_rgb(255_255_255/0.08),0_24px_60px_-20px_rgb(0_0_0/0.8)] backdrop-blur-3xl backdrop-saturate-[1.8] lg:block">
        <Sidebar {...sidebarProps} />
      </aside>

      <header className="sticky top-0 z-30 flex h-12 items-center justify-between border-b border-white/[0.08] bg-black/40 px-4 backdrop-blur-3xl backdrop-saturate-[1.8] lg:hidden">
        <Link
          href="/dashboard"
        ...
        {menuOpen && (
          <div key="mobile-nav" className="fixed inset-0 z-40 lg:hidden">
            ...
            <motion.div
              ...
              className="glass-rim absolute inset-y-2 left-2 w-72 max-w-[calc(100vw-5rem)] rounded-[26px] bg-[#101012]/70 shadow-[0_24px_60px_-12px_rgb(0_0_0/0.9)] backdrop-blur-3xl backdrop-saturate-[1.8]"
              initial={{ x: "-110%" }}
              animate={{ x: 0 }}
