...
export function FocusBanner({ awaiting }: { awaiting: Incident[] }) {
  ...
  if (!top) {
    return (
      <section className="surface relative overflow-hidden rounded-3xl p-6 animate-in fade-in animation-duration-700 fill-mode-both sm:p-7">
        <div
          aria-hidden
          className="pointer-events-none absolute inset-0 bg-[radial-gradient(60%_120%_at_0%_0%,rgb(48_209_88/0.12),transparent_60%)]"
        />
        <div className="relative flex items-center gap-4">
          <IconTile icon={ShieldCheck} color="green" size="lg" />
          <div>
            <h2 className="text-[19px] font-semibold tracking-[-0.02em]">You&apos;re all caught up</h2>
  ...
  return (
    ...
    >
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(60%_120%_at_0%_0%,rgb(255_214_10/0.13),transparent_60%)]"
      />
      <div className="relative flex flex-col gap-5 sm:flex-row sm:items-center">
        <IconTile icon={Hourglass} color="yellow" size="lg" />
        <div className="min-w-0 flex-1">
          <h2 id="focus-heading" className="text-[19px] font-semibold tracking-[-0.02em] text-balance">
