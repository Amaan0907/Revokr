...
// Signal-strength bars, so severity reads without relying on colour alone.
  ...
  return (
    <span aria-hidden className={cn("inline-flex h-2.5 items-end gap-px", className)}>
      {BAR_HEIGHTS.map((height, i) => (
        <span
          key={height}
          className={cn("w-[3px] rounded-full", height, i < level ? "bg-current" : "bg-current/25")}
        />
      ))}
    </span>
  );
...
export function SeverityBadge({ severity, className }: { severity: Severity; className?: string }) {
  ...
  return (
    <span className={cn(BADGE_BASE, meta.text, meta.bg, className)}>
...
export function SimulatedBadge({ className }: { className?: string }) {
  return (
    <span
      ...
      className={cn(BADGE_BASE, "bg-simulation/15 text-simulation", className)}
    >
...
export function StatusBadge({ status, className }: { status: IncidentStatus; className?: string }) {
  ...
  return (
    <span title={meta.description} className={cn(BADGE_BASE, meta.text, meta.bg, className)}>
