...
// page and make it unreadable. Pages with less room to spare, like sign-in, pass a lower
// `strength` (0 to 1) to scale the whole arc down.
export function PageGradient({ strength = 1 }: { strength?: number }) {
  const { scrollY } = useScroll();
  const arcOpacity = useTransform(scrollY, (y) => {
    const screen = typeof window === "undefined" ? 800 : window.innerHeight;
    return strength * (1 - 0.82 * Math.min(y / screen, 1));
  });
