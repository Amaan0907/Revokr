...
| Dashboard | Incidents, incident detail, overview and audit pages read the real API when configured. **Sample data is only for the demo sign-in and for running with no backend**: a GitHub or Google sign-in never sees made-up incidents or repositories. The Overview lists only the projects: a card per connected repository, with its open-incident count, and a **Create new project** card that opens the GitHub App install page. Settings has a **Delete account** option (type your username to confirm) that removes the account's stored data and signs you out; it does not uninstall the GitHub App. Each project's own page holds its incident summary (approvals waiting, headline numbers, pipeline, activity), repository details and incident list. The Repositories page lists the repositories the signed-in GitHub account installed the App on (`GET /api/repositories`); each repository has a detail page (installation, incidents, and GitHub facts read with your own sign-in token); the Onboarding page has no API behind it, so a real sign-in gets its setup checklist with nothing marked done. The demo sign-in is open to anyone and, unless `ADMIN_LOGINS` is set, can read incident data once the dashboard is connected to the API (it can only approve simulated incidents). Setting `ADMIN_LOGINS` limits real data to the listed accounts: other GitHub and Google accounts then see no incidents, and the demo sign-in sees sample data. |
