...
| **Onboarding page has no real state** | `GET /api/repositories?installer_github_id=` ([repositories.go](internal/githubapp/repositories.go)) lists the repositories registered for one GitHub account, and the Repositories page uses it for GitHub sign-ins. `GET /api/repositories/:id` returns one such repository with its incidents, and answers 404 for another account's repository. The repository page adds the description, visibility, default branch, language, last push and five recent commits, read from GitHub with the signed-in user's own token, and shows a dash when GitHub won't answer. Nothing on the Onboarding page reads real state, and the monitor and production toggles of the sample view are not part of the real list. Google sign-ins have no GitHub id, so they see an empty list. | The list depends on the installation webhook having been recorded, and only shows repositories saved under the signed-in GitHub account. |
...
| **Account deletion is untested against a real database** | `DELETE /api/account?github_user_id=` ([account.go](internal/githubapp/account.go)), called by the dashboard's Settings > Delete account after it checks the session and a typed confirmation, removes the user, installations, repositories and every incident, action and audit entry under them in one transaction. The input handling is unit-tested; the SQL is not run against PostgreSQL in any test. It does not uninstall the GitHub App, so a later `installation_repositories` event from GitHub would register the repositories again. It does not undo a rotation already carried out. A Google sign-in has nothing stored, so deleting it only signs out. | The audit log is described as append-only; deleting an account is the one way its entries go. Uninstalling the App is left to the user (github.com/settings/installations). |
